...
_Update:_ I upgraded the feature extraction backbone from C3D to I3D to get better spatial-temporal features, and added a Temporal Graph Convolutional Network (T-GCN) to the MIL neural network to solve the temporal flickering problem and handle the new 1024-D inputs. This also improves the efficiency of the model.
...
- **MIL Classifier:** Uses a Temporal Graph Convolutional Network (T-GCN) and a Multiple Instance Learning (MIL) ranking framework to score each segment between 0.0 and 1.0. The T-GCN captures temporal dependencies between adjacent video segments to reduce temporal flickering. The input layers are tailored for the 1024-D I3D features.
